...
import LeadImporter from "@/app/dashboard/components/LeadImporter";
...
function StepWelcomeSignup({ isLoggedIn, onNext, onSignedUp }) {
  ...
  const handleSignUp = async (e) => {
    ...
    // Create coach record via server-side API (bypasses RLS)
    if (data.user) {
      try {
    ...
    if (data.session) {
      // No email confirmation — user is signed in, advance wizard
      onSignedUp({
        id: data.user.id,
        email: email.trim().toLowerCase(),
        full_name: fullName.trim(),
    ...
    <div>
      ...
      <form onSubmit={handleSignUp} className="space-y-4">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-[#E8735A] hover:bg-[#d4634d] disabled:opacity-50 text-white px-4 py-3.5 rounded-xl text-lg font-bold transition-all duration-150 active:scale-95 shadow-sm"
        >
...
export default function OnboardingPage() {
  ...
  const checkAuth = async () => {
    ...
    if (user) {
      ...
      if (profile) {
        ...
        setCoach(profile);
        // Logged-in user who hasn't completed onboarding: start at step 1 (welcome)
      } else {
        // Logged in but no coach record (edge case) — create one
        try {
          await fetch("/api/auth/create-coach", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    // If not logged in, we show step 1 with signup form
    setLoading(false);
  };
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setErrorMsg(data.error || "Failed to complete onboarding. Please try again.");
        setFinishing(false);
        return;
      }
